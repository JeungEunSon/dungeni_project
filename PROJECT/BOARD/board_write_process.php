<?php
    ...
    if(empty($_FILES['upload_file']['name'])){
        $file_name = "null.txt";
        $sql = "INSERT INTO board (title,content,created,user_id,user_name,file_name,w_pwd)
                VALUES ('".$title."','".$content."',sysdate(),'".$user_id."','".$user_name."','".$file_name."',password('".$w_pwd."'))";
        ...
        if(preg_match("/[ㄱ-힣]/", $_FILES['upload_file']['name'])){
            ...
            if($file_error > 0){
                ...
                $sql = "INSERT INTO board (title,content,created,user_id,user_name,file_name,w_pwd)
                        VALUES ('".$title."','".$content."',sysdate(),'".$user_id."','".$user_name."','".$file_name."',password('".addslashes($w_pwd)."'))";
                mysqli_query($con,$sql);
                header("Location:/PROJECT/BOARD/board.php");
            }
        }
