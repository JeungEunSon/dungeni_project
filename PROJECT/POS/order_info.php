       ...
       <div style="position: absolute; left: 50%;">
           <div id="center" style="position: relative; left: -50%; margin-top:-40px;">
					 ...
					 <caption><h2><a href="order_info.php">덩으니나라 POS</a></h2></caption>
					  <br/>
						 ...
						 <tr>
							 <td colspan="2">
								 <div id="receipt" style="cursor:pointer">
									 <?php
										// DB에서 같은 ord_num끼리 묶어 하나의 영수증으로 처리하는 부분 (첫번쨰 같은 날짜로 묶고 그 다음에 주문번호로 묶기)
										$sql = "SELECT substr(cal_cmpt,1,10) as cal_cmpt FROM user_chk WHERE cal_yn='Y' AND refund_yn='N' GROUP BY substr(cal_cmpt,1,10)";
										$result = mysqli_query($con,$sql);
										while($row = mysqli_fetch_assoc($result)){
											echo '<li style="margin-bottom:5px;"><a href="order_info.php?id='.htmlspecialchars($row['cal_cmpt']).'">'.htmlspecialchars($row['cal_cmpt']).'</a></li>'."\n";
										}
									 ?>
									 <br/>
									 <div id="rc_oc">
									 <?php
                                        // 같은 날짜로 한번 GROUP BY
                                        if($_GET['id']){
                                            echo "※ ".$_GET['id']." 결제내역"."<br/><br/>";
                                            $sql = "SELECT ord_num FROM user_chk WHERE cal_yn='Y' AND refund_yn='N' AND substr(cal_cmpt,1,10) = '".$_GET['id']."' GROUP BY ord_num";
                                            $result = mysqli_query($con,$sql);
                                            while($row = mysqli_fetch_assoc($result)){
                                                echo '<li style="margin-bottom:5px;"><a href="order_info.php?id='.$_GET['id'].'&ord_num='.htmlspecialchars($row['ord_num']).'">'."주문번호 ".htmlspecialchars($row['ord_num'])."의 영수증".'</a></li>'."\n";
                                            }
    										echo "<br/>";

                                            // 영수증링크 클릭시 해당 주문번호에 대한 상품리스트 출력하는 부분
    										if(empty($_GET['ord_num']) === false){
    											echo "※ 주문번호 ".$_GET['ord_num']."에 대한 상품리스트"."<br/><br/>";
    											$sql = "SELECT * FROM user_chk WHERE cal_yn='Y' AND refund_yn='N' AND ord_num = '".$_GET['ord_num']."' AND substr(cal_cmpt,1,10)='".$_GET['id']."'";
    											$result = mysqli_query($con,$sql);
    											while($row = mysqli_fetch_assoc($result)){
    											...
    											// 합계는 한번만 출력되도록 한다.
    											$sql = "SELECT * FROM user_chk WHERE cal_yn='Y' AND refund_yn='N' AND ord_num = '".$_GET['ord_num']."' ORDER BY id DESC limit 1";
    											$result = mysqli_query($con,$sql);
                         ...
                         <tr>
                          ...
                          <tr>
                              <td colspan="2">
 								 <div id="refund_date" style="cursor:pointer">
 									 <?php
 										// DB에서 같은 ord_num끼리 묶어 하나의 영수증으로 처리하는 부분 (첫번쨰 같은 날짜로 묶고 그 다음에 주문번호로 묶기)
 										$sql = "SELECT substr(refund_date,1,10) as refund_date FROM user_chk WHERE cal_yn='Y' AND refund_yn='Y' GROUP BY substr(refund_date,1,10)";
 										$result = mysqli_query($con,$sql);
 										while($row = mysqli_fetch_assoc($result)){
 											echo '<li style="margin-bottom:5px;"><a href="order_info.php?r_id='.htmlspecialchars($row['refund_date']).'">'.htmlspecialchars($row['refund_date']).'</a></li>'."\n";
 										}
 									 ?>
 									 <br/>
 									 <div id="refund_list">
 									 <?php
                                         // 같은 날짜로 한번 GROUP BY
                                         if($_GET['r_id']){
                                             echo "※ ".$_GET['r_id']." 환불내역"."<br/><br/>";
                                             $sql = "SELECT ord_num FROM user_chk WHERE cal_yn='Y' AND refund_yn='Y' AND substr(refund_date,1,10) = '".$_GET['r_id']."' GROUP BY ord_num";
                                             $result = mysqli_query($con,$sql);
                                             while($row = mysqli_fetch_assoc($result)){
                                                 echo '<li style="margin-bottom:5px;"><a href="order_info.php?r_id='.$_GET['r_id'].'&r_ord_num='.htmlspecialchars($row['ord_num']).'">'."주문번호 ".htmlspecialchars($row['ord_num'])."의 영수증".'</a></li>'."\n";
                                             }
     										echo "<br/>";

                                             // 영수증링크 클릭시 해당 주문번호에 대한 상품리스트 출력하는 부분
     										if(empty($_GET['r_ord_num']) === false){
     											echo "※ 주문번호 ".$_GET['r_ord_num']."에 대한 환불내역"."<br/><br/>";
     											$sql = "SELECT * FROM user_chk WHERE cal_yn='Y' AND refund_yn='Y' AND ord_num = '".$_GET['r_ord_num']."' AND substr(refund_date,1,10)='".$_GET['r_id']."'";
     											$result = mysqli_query($con,$sql);
     											while($row = mysqli_fetch_assoc($result)){
     												echo "<li>".htmlspecialchars($row['gd_nm']).' '.htmlspecialchars(number_format($row['gd_amt'])).'원 '.htmlspecialchars($row['gd_qnt']).'개 '.htmlspecialchars(number_format($row['gd_amt']*$row['gd_qnt']))."원</li>"."\n";
     											}
     											...
     											// 합계는 한번만 출력되도록 한다.
     											$sql = "SELECT * FROM user_chk WHERE cal_yn='Y' AND refund_yn='Y' AND ord_num = '".$_GET['r_ord_num']."' ORDER BY id DESC limit 1";
     											$result = mysqli_query($con,$sql);
     											$row = mysqli_fetch_assoc($result);
     											$refund_date = htmlspecialchars($row['refund_date']); // 환불처리일시
                                                $cal_cmpt = htmlspecialchars($row['cal_cmpt']); // 구매완료일시
     											$user_num = htmlspecialchars($row['user_num']); // 회원번호
     											$mem_yn = htmlspecialchars($row['dic_yn']); // 멤버십 할인 여부
     											$hap_amt = htmlspecialchars($row['hap_amt']); // 구매금액
              ...
              <script type="text/javascript">
                ...
                //             //   submenu.slideUp();
                //             location.href="/PROJECT/POS/order_info.php";
                //           }else{
                //               submenu.slideDown();
                //           }
                //       });
